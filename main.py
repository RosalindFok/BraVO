...
def main() -> None:
    ...
    ## Path to save
    # the path of hdf5 file: labels, train or test
    ...
    for hdf5_path, trial_path_dict in zip([train_hdf5_path      , test_hdf5_path      ], 
    ...
    if task == 't':
        ...
        train_dataloader = DataLoader(dataset=NSD_HDF5_Dataset(hdf5_path=train_hdf5_path), batch_size=batch_size, shuffle=False, num_workers=num_workers)
        test_dataloader  = DataLoader(dataset=NSD_HDF5_Dataset(hdf5_path=test_hdf5_path),  batch_size=batch_size, shuffle=False, num_workers=num_workers)
        # Loss function
        decoder_loss = Decoder_loss(w1=1, w2=0.5) 
        # Network
        light_loader = next(iter(test_dataloader))
        for tower in ['image', 'caption']:
            if tower == 'image':
            ...
            for epoch in range(epochs):
                ...
                print(f'Tower {tower}, Epoch {epoch+1}/{epochs}, GPU memory usage: {mem_reserved:.2f} GB / {total_memory:.2f} GB.')
                ...
                print(f'Train {decoder_loss.__class__.__name__} = {train_loss:.4f}, Valid MSELoss = {MSELoss:.4f}.')
            ...
            plt.savefig(join_paths(saved_subj_train_result_dir_path, f'brain_{tower}_decoder_valid_mseloss.png'))
            plt.close()
    ...
    # Generate
        ...
        for dir_path in test_dirs_path_list:
            ...
            brovo_cap = np.load(join_paths(dir_path, 'brovo_cap.npy'), allow_pickle=True)
            ...
            hidden_state_dict = {
                ...
                'caption' : __concatenate_embeddings__(img_emb=blip_img , txt_emb=brovo_cap),
                'image'   : __concatenate_embeddings__(img_emb=bravo_img, txt_emb=blip_cap ),
                'img+cap' : __concatenate_embeddings__(img_emb=bravo_img, txt_emb=brovo_cap)
            }
            images_dict = {
                'coco' : coco
            }
